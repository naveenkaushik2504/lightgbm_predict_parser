...
def GetScore(data,model):
    ...
    #             print 'data is '+str(data[i][split_feature[node]])
            if(data[model['split_feature'][i][node]+1] <= model['threshold'][i][node] ):
...
def GetModel(model_file):
    ...
    model.append(tree)
    model_dict = GetModelDetails(model)
    #print len(model)
    return model_dict
...
def score_data(data_file,model):
    #Read the data
    #data_file='data_lgbm.txt'
    score_list_tree = []
    with open(data_file) as f:
        for line in f:
            data = [float(x) for x in line.strip('\n').split(',')]
            member_id = str(int(data[0]))
            score = GetScore(data,model)
            #score = Sigmoid_Transformation(score)
            score_list_tree.append(member_id+','+str(score))
